...
class owner_ptr final : public internal::smart_ptr::owner_ptr_base<T>
{
...
public:
	using base_type = internal::smart_ptr::owner_ptr_base<T>;
...
public:
	_CONSTEXPR20_ owner_ptr(const Allocator& allocator_p = Allocator()) noexcept : base_type(), m_allocator(allocator_p) {}

	_CONSTEXPR23_ ~owner_ptr() noexcept
	{
	...
	_CONSTEXPR20_ owner_ptr(const owner_ptr&) noexcept = delete;

	_CONSTEXPR20_ owner_ptr(owner_ptr&& rvalue_p) noexcept : base_type(rvalue_p.m_smart_ptr), m_allocator(rvalue_p.m_allocator)
	{
	...
	_CONSTEXPR20_ owner_ptr(element_type value_p, const Allocator& allocator_p = Allocator()) noexcept : m_allocator(allocator_p)
	{
	...
	_CONSTEXPR23_ owner_ptr& operator=(const owner_ptr&) noexcept = delete;

	_CONSTEXPR20_ owner_ptr& operator=(owner_ptr&& rvalue_p) noexcept
	{
		if ((rvalue_p.m_smart_ptr == nullptr) || (rvalue_p.m_key == base_type::invalid_key_value))
		{
			return *this;
	...
	_CONSTEXPR20_ owner_ptr& operator=(element_type value_p) noexcept
	{
		if (this->m_smart_ptr == nullptr)
		{
	...
	_CONSTEXPR20_ void swap(owner_ptr& in_out_other_p) noexcept
	{
		owner_ptr l_tmp = std::move(in_out_other_p);
	...
	_FORCE_INLINE_ boolean operator==(const owner_ptr& other_p) const noexcept
	{
	...
	_FORCE_INLINE_ boolean operator==(const safe_ptr<T>& other_p) const noexcept
	{
		return this->m_smart_ptr == other_p.m_smart_ptr;
	}
...
_CONSTEXPR23_ _NODISCARD_ owner_ptr<T, Allocator> make_owner() noexcept
{
	static_assert(std::is_array<T>::value == false, "static assertion failed: The typename T must not be an array type");
	return owner_ptr<T, Allocator>(T());
}

template<typename T, class Allocator = FE::new_delete_allocator<FE::aligned_allocator<T>>>
_CONSTEXPR23_ _NODISCARD_ owner_ptr<T, Allocator> make_owner(T value_p) noexcept
{
	static_assert(std::is_array<T>::value == false, "static assertion failed: The typename T must not be an array type");
	return owner_ptr<T, Allocator>(T(std::move(value_p)));
}
...
class owner_ptr<T[], Allocator> final : public internal::smart_ptr::owner_ptr_base<T[]>
{
	FE_STATIC_ASSERT(std::is_class<Allocator>::value == false, "Static Assertion Failed: The template argument Allocator is not a class or a struct type.");
	FE_STATIC_ASSERT((std::is_same<T, Allocator::value_type>::value == false), "static assertion failed: enforcing Allocator's value_type to be equivalent to the typename T. The template parameter T must be identical to the value_type of the Allocator.");
...
public:
	_CONSTEXPR17_ owner_ptr(const Allocator& allocator_p = Allocator()) noexcept : base_type(), m_smart_ptr_end(base_type::m_smart_ptr), m_allocator(allocator_p) {}
	...
	_CONSTEXPR20_ owner_ptr(owner_ptr&& rvalue_p) noexcept : base_type(rvalue_p.m_smart_ptr), m_smart_ptr_end(rvalue_p.m_smart_ptr_end), m_allocator()
	{
		if ((rvalue_p.m_smart_ptr == nullptr) || (rvalue_p.m_key == base_type::invalid_key_value))
		{
			return;
	...
	_CONSTEXPR20_ owner_ptr(FE::reserve&& array_size_p, const Allocator& allocator_p = Allocator()) noexcept : m_allocator(allocator_p)
	{
	...
	_CONSTEXPR20_ owner_ptr(std::initializer_list<element_type>&& values_p, const Allocator& allocator_p = Allocator()) noexcept : m_allocator(allocator_p)
	{
	...
	_CONSTEXPR20_ owner_ptr& operator=(std::initializer_list<element_type>&& values_p) noexcept
	{
		if (values_p.size() == 0)
		{
			return *this;
	...
	_CONSTEXPR20_ owner_ptr& operator=(FE::resize_to&& new_array_size_p) noexcept
	{
...
template<typename T, class Allocator = FE::new_delete_allocator<FE::aligned_allocator<typename std::remove_all_extents<T>::type>>>
_CONSTEXPR23_ _NODISCARD_ owner_ptr<typename std::remove_all_extents<T>::type[], Allocator> make_owner(size_t array_size_p) noexcept
{
	static_assert(std::is_array<T>::value == true, "static assertion failed: The typename T must be an array type");
	return owner_ptr<typename std::remove_all_extents<T>::type[], Allocator>(FE::reserve{ array_size_p });
}

template<typename T, class Allocator = FE::new_delete_allocator<FE::aligned_allocator<typename std::remove_all_extents<T>::type>>>
_CONSTEXPR23_ _NODISCARD_ owner_ptr<typename std::remove_all_extents<T>::type[], Allocator> make_owner(std::initializer_list<typename std::remove_all_extents<T>::type>&& values_p) noexcept
{
	static_assert(std::is_array<T>::value == true, "static assertion failed: The typename T must be an array type");
	return owner_ptr<typename std::remove_all_extents<T>::type[], Allocator>(std::move(values_p));
}
