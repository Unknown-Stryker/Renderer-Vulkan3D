...
window::window(uint32 width_p, uint32 height_p, const char* title_p) noexcept : m_width(width_p), m_height(height_p), m_window(), m_title(title_p), m_pipeline()
{
	...
	constexpr length_t l_path_to_vert_shader_len = algorithm::string::length(_CMAKE_PROJECT_DIRECTORY_) + algorithm::string::length("shaders/bin/sample_shader.vert.spv");
	FE::fstring<l_path_to_vert_shader_len> l_path_to_vert_shader(_CMAKE_PROJECT_DIRECTORY_);

	constexpr length_t l_path_to_frag_shader_len = algorithm::string::length(_CMAKE_PROJECT_DIRECTORY_) + algorithm::string::length("shaders/bin/sample_shader.frag.spv");
	FE::fstring<l_path_to_frag_shader_len> l_path_to_frag_shader(_CMAKE_PROJECT_DIRECTORY_);

	l_path_to_vert_shader += "shaders/bin/sample_shader.vert.spv";
	l_path_to_frag_shader += "shaders/bin/sample_shader.frag.spv";
